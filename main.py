...
def ur(pid, res):
    try:
        with db() as c:
            c.execute("UPDATE p SET res=? WHERE id=?", (res, pid))
            c.commit()
    except:
        pass
...
def rdb():
    try:
        with db() as c:
            c.execute("DROP TABLE IF EXISTS p")
            c.commit()
            db()
    except:
        pass

# CSS MIARAKA AMIN'NY STYLE PLAIN BLACK HO AN'NY PLACEHOLDER
st.markdown("""
<style>
...
/* STYLING INPUTS SY PLACEHOLDER MAINY SY NGEZA */
...
# AMBOARINA NY CALCUL ENTRY TIME MBA TSY HO TARA
def calc_entry(hn, bp4, str_, lc, last_time_str):
    try:
        parts = last_time_str.strip().split(":")
        # Maka ny ora sy minitra ary segondra avy amin'ny input
        if len(parts) == 3:
        ...
        # Mamorona object datetime miorina amin'ny ora nampidirina (fa tsy ny ora izao)
        base_time = datetime.now(TZ).replace(hour=h, minute=m, second=s, microsecond=0)
        
    except:
        base_time = datetime.now(TZ)

    # Elanelana segondra (Logic sniper)
    hv = (hn % 60) - 30
    pb = int((bp4 - 30) * 0.45)
    sb = int((str_ - 50) * 0.25)
    cb = int(lc * 3.0)
    ...
    # Ny Entry Time dia ny Ora taloha + ilay elanelana sh
    entry_datetime = base_time + timedelta(seconds=sh)
    
    return entry_datetime.strftime("%H:%M:%S"), sh
...
if not st.session_state.auth:
    ...
    with cb:
        ...
        pw = st.text_input("🔑 MOT DE PASSE", type="password", placeholder="COSMOS2026")
...
with st.sidebar:
    ...
    if st.button("🗑️ RESET", use_container_width=True):
        rdb(); st.session_state.R = None; st.session_state.pid = None; st.rerun()
...
with ci:
    st.markdown("<div class='card'>", unsafe_allow_html=True)
    sha5 = st.text_input("🔐 SHA512", placeholder="AC50E")
    hex8 = st.text_input("🔢 HEX 8", placeholder="7DB8E014")
    lc = st.number_input("📊 LAST COTE", value=1.88, step=0.01, format="%.2f")
    lt_in = st.text_input("⏰ LAST TIME", placeholder="20:22:24")
    st.markdown("</div>", unsafe_allow_html=True)
    
    if st.button("🎯 SNIPER OMEGA", use_container_width=True):
        if sha5 and hex8 and lt_in:
            with st.spinner("🎯 500k sims targeting..."):
    ...
    if r:
        ...
        with cw:
            if st.button("✅ WIN", use_container_width=True, key="bw"):
                ur(st.session_state.pid, "WIN"); st.rerun()
        with cl2:
            if st.button("❌ MISS", use_container_width=True, key="bl"):
                ur(st.session_state.pid, "LOSS"); st.rerun()
        st.markdown("</div>", unsafe_allow_html=True)
    else:
        st.markdown("<div class='card' style='min-height:380px;display:flex;align-items:center;justify-content:center;'><div style='text-align:center;'><div class='x4big' style='font-size:3rem;opacity:.2;'>X4</div><div style='color:rgba(255,255,255,.15);font-family:Orbitron;margin-top:10px;'>MIANDRY INPUT...</div></div></div>", unsafe_allow_html=True)
