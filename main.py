...
/* INPUTS STYLING */
.stTextInput input, .stNumberInput input{background:#ffffff!important;border:2px solid #00ffcc!important;color:#000000!important;font-weight:900!important;border-radius:11px!important;font-family:'Orbitron',sans-serif!important;}
.stTextInput input::placeholder{color:#000000!important;opacity:0.6!important;}
.stButton>button{background:linear-gradient(135deg,#00ffcc,#009977)!important;color:#000!important;font-weight:900!important;border-radius:12px!important;height:52px!important;font-family:'Rajdhani'!important;transition:all .2s!important}
...
# CALCOL HEURE D'ENTRÉE AMBOARINA (DELAY + MALALAKA)
def calc_entry(hn, bp4, str_, lc, last_time_str):
    ...
    # NOVANA: Min 45s ka hatramin'ny 180s mba ho tratra tsara
    sh=max(45, min(180, 80+hv+pb+sb+cb))
...
def engine(sha5, hex8, lc, last_time):
    ...
    if len(sx4)>0:
        t4min,t4moy,t4max=round(float(np.percentile(sx4,15)),2), round(float(np.percentile(sx4,50)),2), round(float(np.percentile(sx4,85)),2)
        acc4min,acc4moy,acc4max=round(p4*0.85,1), round(p4*0.50,1), round(p4*0.15,1)
    else:
    ...
    str_=max(25.0,min(99.0,round(bp4*0.45+p45*0.25+p5*0.15+p3*0.05+(hn%200)/14+(hp/100)*12,1)))
    ent,sh=calc_entry(hn,bp4,str_,lc,last_time)

    if   str_>=90 and bp4>=28: sig,sc="🎯🎯🎯 OMEGA X4+ — FIRE!","sig-u"
    elif str_>=78 and bp4>=22: sig,sc="🎯🎯 STRONG X4+ — FIRE!","sig-u"
    elif str_>=65 and bp4>=16: sig,sc="🔥 X4+ POSSIBLE — GO","sig-s"
    elif str_>=52 and bp4>=11: sig,sc="🟡 X4 WEAK — MICRO","sig-w"
    else:                      sig,sc="⚠️ SKIP — NO SIGNAL","sig-x"

    return {"ts":datetime.now(TZ).strftime("%Y-%m-%d %H:%M:%S"),"sha5":sha5,"hex8":hex8[:8],"lc":lc,"lt":last_time,"ent":ent,"sh":sh,"sig":sig,"sc":sc,"bp4":bp4,"p4":p4,"p45":p45,"p5":p5,"p6":p6,"p3":p3,"str":str_,"state":cur,"hp":hp,"t4min":t4min,"t4moy":t4moy,"t4max":t4max,"acc4min":acc4min,"acc4moy":acc4moy,"acc4max":acc4max}
...
if not st.session_state.auth:
    ...
    with cb:
        ...
        pw=st.text_input("🔑 CODE D'ACCÈS",type="password",placeholder="Entrez: COSMOS2026")
...
with st.sidebar:
    ...
    if st.button("🗑️ RESET DATA"): rdb(); st.rerun()
...
with ci:
    st.markdown("<div class='card'>",unsafe_allow_html=True)
    sha5  = st.text_input("🔐 SHA512 (5 decimals)",placeholder="Ex: ac50e")
    hex8  = st.text_input("🔢 HEX / ROUND ID",placeholder="Ex: 7db8e014")
    ...
    if st.button("🎯 CALCULER SIGNAL",use_container_width=True):
        if sha5 and hex8 and lt_in:
            r=engine(sha5.strip(),hex8.strip(),lc,lt_in.strip())
            st.session_state.R=r; st.session_state.pid=sp(r); st.rerun()
    st.markdown("</div>",unsafe_allow_html=True)
...
with co:
    ...
    if r:
        ...
        st.markdown(f"<p style='text-align:center;color:rgba(255,255,255,.4);margin-top:10px;'>▸ HEURE D'ENTRÉE JETX / AVIATOR</p>",unsafe_allow_html=True)
        ...
        <span class='tag-y'>X5+ {r['p5']}%</span><span class='tag-y'>X6+ {r['p6']}%</span>
        </div>""",unsafe_allow_html=True)
        st.markdown("<br>",unsafe_allow_html=True)
        cw,cl2=st.columns(2)
        with cw:
            if st.button("✅ WIN X4+",use_container_width=True): ur(st.session_state.pid,"WIN"); st.rerun()
        with cl2:
            if st.button("❌ MISS",use_container_width=True): ur(st.session_state.pid,"LOSS"); st.rerun()
        st.markdown("</div>",unsafe_allow_html=True)
    else:
        st.markdown("<div class='card' style='min-height:380px;display:flex;align-items:center;justify-content:center;opacity:0.3;'>🎯 MIANDRY INPUT...</div>",unsafe_allow_html=True)
